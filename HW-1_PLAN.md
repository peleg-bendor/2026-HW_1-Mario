...
Shared working notes for building Exercise 1. The assistant may edit this file directly as we go — it's not "game code," it's our notes.
...
0. `[ ]` **Stage 0 — Clean up & steady base.** Before writing new features, go through the inherited Lesson 4 code together and decide what (if anything) to fix:
   - `WeaponsHandler.index` is hardcoded to `0` → the Axe is currently unreachable via input (only the Fireball fires). Decide: fix now, or defer to the Pickable Axe stage (it naturally belongs there too).
   ...
   - Decide what to do with `SampleScene.unity` (unused default template scene) and fix Build Settings accordingly (currently it only lists `SampleScene.unity`, not the real scene).
1. `[ ]` Coin-count GUI — already exists (`SC_CoinsManager` + `Txt_Coins`); confirm it still works after Stage 0 changes.
...
_(append dated entries here as we make design decisions, e.g. "2026-08-03: chose to model lives via a GameManager singleton because ...")_

## Notes for the Video

_(build this up per stage, so recording is just following a checklist at the end — not a scramble to remember what to show)_
