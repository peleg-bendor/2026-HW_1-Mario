...
- Per-stage capture notes:
    ...
    - Moving enemy: show it patrolling using the level's actual platform edges/walls, not a fixed distance; show it costing a strike on touch; show it destroyed by both a thrown axe and a fireball. Worth narrating the ISP point on `IEnemy` staying to just `Kill()`, since touch-damage isn't universal to every enemy type.
    - Static enemy:
    - Key + Door:
